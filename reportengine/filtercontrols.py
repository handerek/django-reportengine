"""
Based loosely on admin filterspecs, these are more focused on delivering 
controls appropriate per field type

Different filter controls can be registered per field type. When assembling a 
set of filter controls, these field types will generate the appropriate set of 
fields. These controls will be based upon what is appropriate for that field. 
For instance, a datetimefield for filtering requires a start/end. A boolean 
field needs an "all", "true" or "false" in radio buttons.


TODO: build register and lookup functions
TODO: figure out how to manage filters and actual request params, which aren't 
      always 1-to-1 (e.g. datetime)
"""
...
class FilterControl(object):
    ...
    def __init__(self, field, field_name, label=None):
        self.field = field
    ...
    def create_from_modelfield(cls, f, field_name, label=None):
        for test, factory, datatype in cls.filter_controls:
            if test(f):
                return factory(f, field_name, label)
...
class DateTimeFilterControl(FilterControl):
    ...
    def get_fields(self):
        ...
        start = forms.DateTimeField(label=_("%s From") % ln, required=False,
            widget=forms.DateTimeInput(attrs={'class': 'vDateField'}))
        end = forms.DateTimeField(label=_("%s To") % ln, required=False,
            widget=forms.DateTimeInput(attrs={'class': 'vDateField'}))
...
class ForeignKeyFilterControl(FilterControl):
    ...
    def __init__(self, *args, **kwargs):
        super(ForeignKeyFilterControl, self).__init__(*args, **kwargs)
        self.queryset = self.field.rel.to.objects.all()

    def get_fields(self):
        choices = [ (i.id, unicode(i)) for i in self.queryset ]
        return {
            '%s__id' % self.field_name: forms.TypedChoiceField(choices, label=self.label or self.field_name,
                                                    required=False, coerce=int),
        }
        
FilterControl.register(lambda m: isinstance(m, models.ForeignKey), ForeignKeyFilterControl, "integer")
